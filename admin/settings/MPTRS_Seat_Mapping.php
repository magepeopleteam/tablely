	...
	if (!class_exists('MPTRS_Seat_Mapping')) {
		class MPTRS_Seat_Mapping {
			public function __construct() {
                ...
                add_action('wp_ajax_mptrs_added_seat_mapping_in_restaurant', [$this, 'mptrs_added_seat_mapping_in_restaurant'] );
                add_action('wp_ajax_nopriv_mptrs_added_seat_mapping_in_restaurant', [$this, 'mptrs_added_seat_mapping_in_restaurant'] );
			}

            function mptrs_added_seat_mapping_in_restaurant() {

                if ( !isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'mptrs_admin_nonce') ) {
					wp_send_json_error('Invalid nonce!');
				}
                if ( isset( $_POST['post_id'] ) && isset( $_POST['seat_map_id'] ) ) {
                    $post_id = intval($_POST['post_id']);
                    $seat_map_id = sanitize_text_field($_POST['seat_map_id']);
                    update_post_meta( $post_id, 'mptrs_selected_seat_map', $seat_map_id );

                    wp_send_json_error('Successfully Updated');
                }else {
			 ...
			 public function seat_mapping_tab_content($post_id) {
                ...
                <div class="tabsItem" data-tabs="#mptrs_add_seat_mapping">
                    <header>
                        <h2><?php esc_html_e('Add Created Seat Mapp', 'tablely'); ?></h2>
                        <span><?php esc_html_e('In this section you will select any seat mapped for table and seat for reservation.', 'tablely'); ?></span>
                    </header>
                    <section class="mptrs-seat-mapping-section" id="mptrs-seat-mapping-section">
                        <input type="hidden" id="mptrs_add_mapping_id" value="<?php echo esc_attr( $post_id );?>">
                        <select name="mptrs_seat_mapping_select" id="mptrs_seat_mapping_select">
                            <option id="mptrs_select_mapped_seat" value=""><?php esc_html_e('Select a Seat Mapping', 'tablely'); ?></option>
                            <?php
                            if ( $query->have_posts() ) {
                                while ( $query->have_posts() ) {
                                    ...
                                    $post_id = get_the_ID();
                                    $title   = get_the_title();
                                    $selected = ($post_id == $selected_seat_map_id) ? 'selected' : '';
