			...
			public static function post_select() {
                ...
                <label class="min_400 mptrs_id">
                    <select name="mptrs_id" class="formControl mp_select2" id="mptrs_id" required>
                        <option value="0"><?php esc_html_e('Select', 'tablely') . ' ' . esc_html($label); ?></option>
						<?php
						...
						<?php
							if ($params['pagination-style'] == 'load_more') {
                                ...
                                <button type="button" class="_mpBtn_xs_min_200 pagination_load_more" data-load-more="0">
									<?php esc_html_e('Load More', 'tablely'); ?>
                                </button>
								<?php
							}
							else {
                                ...
                                <div class="buttonGroup">
									<?php if ($total_page > 2) { ?>
                                        <button class="_mpBtn_xs page_prev" type="button" title="<?php esc_html_e('GoTO Previous Page', 'tablely'); ?>" disabled>
									...
									<?php if ($total_page > 2) { ?>
                                        <button class="_mpBtn_xs page_next" type="button" title="<?php esc_html_e('GoTO Next Page', 'tablely'); ?>">
			...
			public static function load_more_text($text = '', $length = 150) {
				...
				if ($text && $text_length > $length) {
                    ...
                    <div class="mp_load_more_text_area">
                        ...
                        <div data-read data-open-text="<?php esc_attr_e('Load More', 'tablely'); ?>" data-close-text="<?php esc_attr_e('Less More', 'tablely'); ?>">
                            <span data-text><?php esc_html_e('Load More', 'tablely'); ?></span>
                        </div>
                    </div>
					<?php
				}
					...
					if ($input_type != 'dropdown') {
                        ...
                        <label>
                            <select name="<?php echo esc_attr($input_name); ?>" data-price="<?php echo esc_attr($price); ?>" class="formControl">
                                <option selected value="0"><?php echo esc_html__('Please select', 'tablely').' '.esc_html($text); ?></option>
								<?php
